...
public:
	...
	bool HandleInput( const float t )
	{
		...
		if (IsKeyDown( GLFW_KEY_UP )) camTarget -= speed * up * 10, changed = true;
		if (IsKeyDown( GLFW_KEY_DOWN )) camTarget += speed * up * 10, changed = true;
		if (IsKeyDown( GLFW_KEY_LEFT )) camTarget -= speed * right * 10, changed = true;
		if (IsKeyDown( GLFW_KEY_RIGHT )) camTarget += speed * right * 10, changed = true;
